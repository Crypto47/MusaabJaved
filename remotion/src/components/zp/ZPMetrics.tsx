...
interface ZPMetricsProps {
  metrics: { value: number; label: string; suffix?: string }[];
}

const COLORS    = ["#4ade80", "#fb923c", "#86efac"];
const BAR_COLORS = ["rgba(74,222,128,0.25)", "rgba(251,146,60,0.25)", "rgba(134,239,172,0.25)"];

function ZPCounter({ value, label, suffix = "", delay, color, barColor }: {
...
export function ZPMetrics({ metrics }: ZPMetricsProps) {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: "18px", letterSpacing: "8px", fontWeight: 700, color: "#4ade80", opacity: labelOpacity }}>
        RESULTS
      </div>
      <div style={{ display: "flex", gap: "100px", flexWrap: "wrap", justifyContent: "center" }}>
        {metrics.map((m, i) => (
          <ZPCounter key={m.label} value={m.value} label={m.label} suffix={m.suffix ?? ""}
