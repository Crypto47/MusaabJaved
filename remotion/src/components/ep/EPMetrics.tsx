...
interface EPMetricsProps {
  metrics: { value: number; label: string; suffix?: string }[];
}

const COLORS = ["#fbbf24", "#14b8a6", "#fcd34d"];
const BAR_COLORS = ["rgba(251,191,36,0.25)", "rgba(20,184,166,0.25)", "rgba(252,211,77,0.25)"];

function EPCounter({ value, label, suffix = "", delay, color, barColor }: {
  ...
  return (
    <div style={{ textAlign: "center", opacity, transform: `translateY(${y}px)`, minWidth: "280px" }}>
      ...
      <div style={{
        ...
        marginTop: "16px", marginBottom: "16px", fontWeight: 500,
      }}>
        {label}
      </div>
...
export function EPMetrics({ metrics }: EPMetricsProps) {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
